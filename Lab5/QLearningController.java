...
public class QLearningController extends Controller {
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			String new_state = StateAndReward.getStateAngle(angle.getValue(), vx.getValue(), vy.getValue()) + StateAndReward.getStateHover(angle.getValue(), vx.getValue(), vy.getValue());
			...
			if (previous_state != null) {
				...
				double currentQ = Qtable.get(prev_stateaction) == null ? 0.0 : Qtable.get(prev_stateaction);
				double maxQ = getMaxActionQValue(new_state);
				double reward = previous_reward;
				double alpha = alpha(Ntable.get(prev_stateaction));
				double updatedQ = currentQ + alpha * (reward + GAMMA_DISCOUNT_FACTOR * maxQ - currentQ);
				Qtable.put(prev_stateaction, updatedQ);
